...
# 服务端按主题关键词过滤未读邮件（避免拉取整箱；发票邮件主题通常含这些词）。
# （“电子发票”是“发票”的子集，故不单列。）
_KEYWORD_SEARCH = (
    "UNSEEN",
    "OR",
...
# 单次同步处理的最大封数（取最新一批），防止大邮箱拖垮 IMAP / 任务
MAX_PER_SYNC = 100
...
async def fetch_unseen_messages(
    ...
    try:
        ...
        resp = await client.uid_search(*_KEYWORD_SEARCH, charset="UTF-8")
        # 只取比上次更新的 UID，按升序取最新一批（最多 MAX_PER_SYNC）
        new_uids = sorted(u for u in _parse_uid_line(resp) if u > last)[-MAX_PER_SYNC:]
